...
class EmmDatabase:
    ...
    def execute(
        self, sql: str, params: tuple = (), one: bool = False, immediate: bool = False
    ):
    ...
    def run_migrations(self):
        ...
        with self.connection() as connection:
            connection.execute(
                "CREATE TABLE IF NOT EXISTS schema_migrations (id INTEGER PRIMARY KEY AUTOINCREMENT, migration_name TEXT UNIQUE, applied_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)"
            )
    ...
    def create_project(self, project_path: str) -> int:
        ...
        row = self.execute(
            "SELECT id FROM projects WHERE hash = ?", (content_hash,), one=True
        )
        if row:
            return row["id"]
        with self.connection() as connection:
            cursor = connection.execute(
                ...
                (content, content_hash, Path(project_path).stem),
            )
    ...
    def get_last_session_id(self, status_not: str = "completed") -> int | None:
        row = self.execute(
            "SELECT id FROM sessions WHERE status != ? ORDER BY created_at DESC LIMIT 1",
            (status_not,),
            one=True,
        )
    ...
    def create_task(self, session_id: int, task_data: dict):
        return self.execute(
            "INSERT INTO tasks (session_id, branch_name, task_id, title, description, acceptance_criteria) VALUES (?, ?, ?, ?, ?, ?)",
            (
                session_id,
                ...
                json.dumps(task_data.get("acceptanceCriteria", [])),
            ),
        )

    def get_highest_task_id(self, session_id: int) -> int:
        row = self.execute(
            "SELECT task_id FROM tasks WHERE session_id = ? ORDER BY task_id DESC LIMIT 1",
            (session_id,),
            one=True,
        )
        try:
    ...
    def get_tasks(self, session_id: int) -> list[dict]:
        return [
            {**dict(row), "acceptance_criteria": json.loads(row["acceptance_criteria"])}
    ...
    def get_next_task(self, session_id: int) -> dict | None:
        row = self.execute(
            "SELECT * FROM tasks WHERE session_id = ? AND status = 'pending' ORDER BY id ASC LIMIT 1",
            (session_id,),
            one=True,
        )
        return (
    ...
    def update_task_status(self, session_id: int, task_id: str, status: str):
        ...
        clause = (
            f"status = ?, {time_column} = CURRENT_TIMESTAMP"
            if time_column
            else "status = ?"
        )
        self.execute(
            f"UPDATE tasks SET {clause} WHERE session_id = ? AND task_id = ?",
            (status, session_id, task_id),
        )

    def create_iteration(
        self, session_id: int, iteration_number: int, task_id_worked_on: str
    ) -> int:
        with self.connection() as connection:
            return connection.execute(
                "INSERT INTO iterations (session_id, iteration_number, task_id_worked_on) VALUES (?, ?, ?)",
                (session_id, iteration_number, task_id_worked_on),
            ).lastrowid

    def update_iteration(self, iteration_id: int, output: str, stderr: str):
        self.execute(
            "UPDATE iterations SET opencode_output = ?, opencode_stderr = ?, completed_at = CURRENT_TIMESTAMP, status = 'completed' WHERE id = ?",
    ...
    def get_iterations_for_task(self, session_id: int, task_id: str) -> list[dict]:
        return [
            dict(row)
            for row in self.execute(
                "SELECT * FROM iterations WHERE session_id = ? AND task_id_worked_on = ? ORDER BY iteration_number ASC",
                (session_id, task_id),
            )
        ]
    ...
    def log_message(
        ...
        self.execute(
            "INSERT INTO console_logs (session_id, iteration_number, log_level, message) VALUES (?, ?, ?, ?)",
            (session_id, iteration_number, log_level, message),
        )

    def claim_next_available_project(self, max_iterations: int) -> int | None:
        with self.connection(immediate=True) as connection:
            row = connection.execute(
                "SELECT id FROM projects WHERE id NOT IN (SELECT project_id FROM sessions) ORDER BY id ASC LIMIT 1"
            ).fetchone()
            if not row:
                return None
            connection.execute(
                "INSERT INTO sessions (project_id, max_iterations) VALUES (?, ?)",
                (row["id"], max_iterations),
            )
    ...
    def validate_project(project_data: dict) -> bool:
        return (
            ...
            and "tasks" in project_data
            and isinstance(project_data["tasks"], list)
        )
